...
export class AuthService {
  ...
  async loginUser(login: LoginUserDto, res: Response) {
    ...
    const jwtSession = sign(authenticatedUser, process.env.JWT_KEY, {
      expiresIn: '4h',
    });
  ...
  async verifyUser(res: Response, req: Request) {
    ...
    if (!token) {
      return res.status(401).json({
        error: 'Token No Valido o Autorizado',
      });
    }
    try {
      ...
      if (typeof decoded === 'object' && decoded.id) {
        ...
        res.json({
          ...
          accountOwner: req.user.accountOwner,
          changedPw: req.user.changedPw,
        });
      }
    } catch (error) {
      console.error(error);
